...
public class DE18B{
  ...
  void countOnes(){  // compute numberOfIncidences and allocate words[]
    ...
    for (int i = 0; i < numberOfDocs; i++) 
      for (int j = 0; j < picLen; j++){
        ...
        if (c < 'a') counts[c - '0']++; else counts[c - 'a' + 10]++;
    }
 ...
 void evaluateCodes(){
   ...
   for (int docID = 0; docID < numberOfDocs; docID++)
     for (int j = 0; j < numberOfTopics; j++){
	   ...
	   // Your code to increment changes and update codes[docID][j]
           changes++; codes[docID][j] = bit;
        }
     }
   System.err.println(changes); // number of bits changed in hash values for all docs
 }

 void displayCodes(){  // print hash values in hex as codes to docs
   for (int docID = 0; docID < numberOfDocs; docID++){
     int n = 0; String code = "";
     for (int i = 0; i < 16; i++){
       int b = 0;
 ...
 public static void main(String[] args){
   DE18B de18 = new DE18B();
   de18.readPics("faces800.txt");
   de18.countOnes();
   de18.makePostings();
   de18.initialize();
   de18.epochs();
